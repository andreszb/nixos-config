...
Why an ~org~ file? My Emacs config is large, and this enables me to improve the readability of its documentation greatly. I edit this file the same way you're reading it (nicely formatted) as I use Emacs and Emacs speaks ~org~. Sounds complicated, but it's really not; [[https://github.com/andreszb/nixos-config/blob/main/hosts/nixos/default.nix#L236][just a few lines]] in my Nix config. [[https://github.com/andreszb/nixos-config/blob/main/hosts/darwin/default.nix#L40][MacOS too.]]

This is the main configuration, but one more init file also exists, ~init.el~ , that bootstraps ~org-mode~ before this file is interpreted. That's defined [[https://github.com/andreszb/nixos-config/blob/main/modules/shared/files.nix#L56][here]].
...
#+BEGIN_SRC emacs-lisp
  (setq user-full-name "andreszb Lyons"
    user-mail-address "andreszb@dlyons.dev")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Set path for darwin
  (when (system-is-mac)
    (setenv "PATH" (concat (getenv "PATH") ":/Users/andreszb/.nix-profile/bin:/usr/bin"))
    (setq exec-path (append '("/Users/andreszb/bin" "/profile/bin" "/Users/andreszb/.npm-packages/bin" "/Users/andreszb/.nix-profile/bin" "/nix/var/nix/profiles/default/bin" "/usr/local/bin" "/usr/bin") exec-path)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org-roam
    :straight (:host github :repo "andreszb/org-roam"
               :branch "master"
